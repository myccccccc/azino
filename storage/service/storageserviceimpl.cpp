...
DEFINE_string(storage_name, "azino_storage",
...
void StorageServiceImpl::Get(::google::protobuf::RpcController* controller,
    ...
    if (ss.error_code() != StorageStatus::Ok) {
        ...
        response->set_allocated_status(ssts);
        LOG(WARNING) << cntl->remote_side()
                     << " Fail to get key: " << request->key()
                     << " error code: " << ss.error_code()
                     << " error message: " << ss.error_message();
    } else {
        LOG(INFO) << cntl->remote_side()
                  << " Success to get key: " << request->key()
                  << " value: " << value;
...
void StorageServiceImpl::Delete(::google::protobuf::RpcController* controller,
    ...
    if (ss.error_code() != StorageStatus::Ok) {
        ...
        response->set_allocated_status(ssts);
        LOG(WARNING) << cntl->remote_side()
                     << " Fail to delete key: " << request->key()
                     << " error code: " << ss.error_code()
                     << " error message: " << ss.error_message();
    } else {
        LOG(INFO) << cntl->remote_side()
                  << " Success to delete key: " << request->key();
    }
}
...
void StorageServiceImpl::MVCCPut(
    ...
    if (ss.error_code() != StorageStatus::Ok) {
        ...
        response->set_allocated_status(ssts);
        LOG(WARNING) << cntl->remote_side()
                     << " Fail to put mvcc key: " << request->key()
                     << " ts: " << request->ts()
                     << " error code: " << ss.error_code()
                     << " value: " << request->value()
                     << " error message: " << ss.error_message();
        ...
        LOG(INFO) << cntl->remote_side()
                  << " Success to put mvcc key: " << request->key()
                  << " ts: " << request->ts() << " value: " << request->value();
    }
}
...
void StorageServiceImpl::MVCCGet(
    ...
    if (ss.error_code() == StorageStatus_Code_Ok ||
        ss.error_code() == StorageStatus_Code_NotFound) {
...
void StorageServiceImpl::MVCCDelete(
    ...
    if (ss.error_code() != StorageStatus::Ok) {
        ...
        response->set_allocated_status(ssts);
        LOG(WARNING) << cntl->remote_side()
                     << " Fail to delete mvcc key: " << request->key()
                     << " ts: " << request->ts()
                     << " error code: " << ss.error_code()
                     << " error message: " << ss.error_message();
    } else {
        LOG(INFO) << cntl->remote_side()
                  << " Success to delete mvcc key: " << request->key()
                  << " ts: " << request->ts();
    }
}
...
void StorageServiceImpl::BatchStore(
    ...
    if (ss.error_code() != StorageStatus::Ok) {
        ...
        response->set_allocated_status(ssts);
        LOG(WARNING) << cntl->remote_side()
                     << " Fail to batch store mvcc data. "
                     << " error code: " << ss.error_code()
                     << " data size: " << request->datas().size()
                     << " error message: " << ss.error_message();
    } else {
        LOG(INFO) << cntl->remote_side()
                  << " Success to batch store mvcc data. "
                  << " data size: " << request->datas().size();
    }
}
